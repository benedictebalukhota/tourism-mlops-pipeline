...
## A note on the deployment SDK

As of mid-2026, Hugging Face requires a paid **PRO** plan to create Docker or
Streamlit-SDK Spaces on free personal accounts. Free accounts can still host
up to 2 **Gradio** Spaces on the free ZeroGPU tier. To keep this project
deployable at zero cost, the live app (`tourism_project/deployment/app.py`)
is built with **Gradio** rather than Streamlit.

- A Streamlit version of the same app is kept at
  `tourism_project/deployment/app_streamlit_legacy.py` for reference.
- The `Dockerfile` is still included to document how the app would be
  containerised under a paid plan; it is not used by the actual Gradio
  Space build, which needs only `app.py` and `requirements.txt`.
- `hosting.py` creates the Space with `space_sdk="gradio"` accordingly.
