...
As of mid-2026, Hugging Face requires a paid **PRO** plan (or a Team/Enterprise
plan) to create Docker-SDK Spaces on personal accounts — free personal
accounts can no longer create Docker Spaces, and can only create Gradio
Spaces on the free ZeroGPU tier if the account is verified and at least 30
days old. This project's `app.py` is a **Streamlit** app deployed via
**Docker**, matching the original assignment brief, which requires an active
PRO subscription on the account behind `HF_TOKEN`.
