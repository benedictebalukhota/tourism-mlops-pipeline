...
NOTE: creating a Docker-SDK Space requires a Hugging Face PRO subscription
(or a Team/Enterprise plan) on the account whose HF_TOKEN is used here.
Free personal accounts cannot create Docker (or Gradio-on-cpu-basic)
Spaces as of mid-2026.
...
def main() -> None:
    ...
    # ------------------------------------------------------------------
    # Set HF_USERNAME as a Space runtime variable.
    #
    # GitHub Actions secrets (HF_USERNAME, HF_TOKEN) are only available
    # while this script runs during CI - they do NOT automatically carry
    # over into the live Space's own runtime environment. Without this
    # step, app.py's `os.getenv("HF_USERNAME", "<your-hf-username>")`
    # would fall back to the literal placeholder string at runtime and
    # fail to find the model repo.
    # ------------------------------------------------------------------
    ...
    print(f"Set HF_USERNAME={HF_USERNAME} as a Space variable")
